...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    GoalMemberList=[[NSMutableDictionary alloc]init];
    selectedGoal=[[NSMutableDictionary alloc]init];
    ...
    self.slideMenuVC.backgroundImage = [UIImage imageNamed:@"menu@3x.png"];
   ...
   // [self.window makeKeyAndVisible];
    UIViewController *localVC=[[UIStoryboard storyboardWithName:@"Main" bundle: nil] instantiateInitialViewController];
    self.VC=(MainViewController*)localVC;
...
- (NSURL *)applicationDocumentsDirectory {
    // The directory the application uses to store the Core Data store file. This code uses a directory named "com.deemsysinc.CyberHealth" in the application's documents directory.
...
- (NSManagedObjectModel *)managedObjectModel {
    ...
    NSURL *modelURL = [[NSBundle mainBundle] URLForResource:@"CyberHealth" withExtension:@"momd"];
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator {
    ...
    NSURL *storeURL = [[self applicationDocumentsDirectory] URLByAppendingPathComponent:@"CyberHealth.sqlite"];
    NSError *error = nil;
...
- (void)setSecondView{
  /*
    if (!altVC) {
        UIStoryboard *mainStoryboard = [UIStoryboard storyboardWithName:@"Dashboard" bundle: nil];
        altVC = [mainStoryboard instantiateViewControllerWithIdentifier:@"HKMenuView"];
    }
...
//    [[UIApplication sharedApplication].keyWindow setRootViewController:self.slideMenuVC];
...
-(void)willOpenMenu{
    NSLog(@"delegate %s", __PRETTY_FUNCTION__);
}

-(void)didOpenMenu{
    NSLog(@"did open %s", __PRETTY_FUNCTION__);
}

-(void)willCloseMenu{
    NSLog(@"will close%s", __PRETTY_FUNCTION__);
}
