...
//	self.navigationItem.rightBarButtonItem = self.editButtonItem;
	...
	_dataModel = [NSMutableArray arrayWithObjects:
				  [NSMutableArray arrayWithObjects:@"Introduction",@"About Course", nil],
				  [NSMutableArray arrayWithObjects:@"The Beginning", @"The Early Years", nil],
				  [NSMutableArray arrayWithObjects:@"Lure Styles & Values", @"Lure Colors", @"Retailers", @"Specials", nil],
				  [NSMutableArray arrayWithObjects:@"Collection Pictures",@"About Author", nil],
				  nil];
...
- (UITableViewCell *)tableView:(ExpandableTableView *)tableView cellForGroupInSection:(NSUInteger)section
{
	...
	//cell.textLabel.text = [NSString stringWithFormat: @"Chapter %d (%d)", section, [self tableView:tableView numberOfRowsInSection:section]];
	
    int tempValue=section+1;
    cell.textLabel.text = [NSString stringWithFormat: @"Chapter %d", tempValue];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    NSLog(@"the values=%@",[[_dataModel objectAtIndex:indexPath.section] objectAtIndex:indexPath.row]);
    [viewController setText:[[_dataModel objectAtIndex:indexPath.section] objectAtIndex:indexPath.row]];
